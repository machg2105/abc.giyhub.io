...
function calculateChange() {
    ...
    // Sumar billetes/monedas usados para pagar
    ...
    // Sumar billetes/monedas del pago al inventario
    paymentDenominations.forEach(denom => {
        const paymentCount = parseInt(prompt(`¿Cuántos ${denom.name} usó para pagar?`, 0));
        if (paymentCount > 0) {
            const currentCount = parseInt(document.getElementById(denom.id).value);
            document.getElementById(denom.id).value = currentCount + paymentCount;
        }
    });
    ...
    // Restar billetes/monedas usados para dar cambio
    denominations.forEach(denom => {
